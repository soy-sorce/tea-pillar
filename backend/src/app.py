...
from src.routers import generate, health, reaction
...
def create_app() -> FastAPI:
    ...
    async def handle_validation_error(
        ...
        error = InvalidInputError(message="入力データに問題があります", detail=str(exc))
        logger.warning("request_validation_error", path=request.url.path, detail=str(exc))
        return JSONResponse(status_code=error.status_code, content=error.to_response_content())
