"""Repository for bandit-related Firestore access."""

from typing import Self

from src.models.firestore import BanditStateDocument, TemplateDocument
from src.services.firestore.client import FirestoreClient


class BanditRepository:
    """Hide Firestore details from the bandit implementation."""

    def __init__(self: Self, firestore_client: FirestoreClient) -> None:
        self._firestore = firestore_client

    async def get_entries_by_state_key(
        self: Self,
        state_key: str,
    ) -> dict[str, BanditStateDocument]:
        return await self._firestore.get_bandit_states_by_state_key(state_key=state_key)

    async def get_active_templates(self: Self) -> list[TemplateDocument]:
        return await self._firestore.get_active_templates()

    async def update_entry(
        self: Self,
        template_id: str,
        state_key: str,
        reward: float,
    ) -> None:
        await self._firestore.update_bandit_state(
            template_id=template_id,
            state_key=state_key,
            reward=reward,
        )
